...
namespace MineS
{
    ...
    {
        const int _rows = 1;
        const int _columns = 1;
        const int _itemLength = 50;
        const int _gridMargin = 50;
        ...
        private void BuildGrid()
        {
            ...
            for (int row = 0; row < _rows; ++row)
            {
                for (int column = 0; column < _columns; ++column, ++imageIdx)
                {
                    ...
                    content.Fill = new SolidColorBrush(Colors.Blue);
                    ...
                    MainCanvas.Children.Add(content2);
